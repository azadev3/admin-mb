...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'controlmeasurecategory', method: 'get' });
  if (!res) return [];
...
const CategoryShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['controlmeasurecategory'],
    queryFn: fetchData,
    retry: 2,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/market-participants/control-measure/category/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint='controlmeasurecategory' />
      <DataTable
        ...
        refetch={refetch}
        onEditLocation={item =>
          `/market-participants/control-measure/category/edit/${item.id}`
        }
        onEdit={() => {}}
        onDelete={() => {}}
      />
