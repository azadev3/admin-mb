...
interface DataInterface {
  id: number;
  controlMeasureCategoryTitle: string;
  month: number;
  year: number;
...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'controlmeasure', method: 'get' });
  if (!res) return [];
...
const ValuesShow: React.FC = () => {
  ...
  const dynamicColumns: Column<DataInterface>[] = [
    ...
    {
      header: 'Kateqoriya',
      accessor: 'controlMeasureCategoryTitle',
      cell: row =>
        row.controlMeasureCategoryTitle ? (
          <Highlighter text={row.controlMeasureCategoryTitle} highlight={searchTerm} />
        ) : (
          <Text>Yoxdur</Text>
        ),
    },
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/market-participants/control-measure/values/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="controlmeasure" />
      <DataTable
        ...
        refetch={refetch}
        onEditLocation={item =>
          `/market-participants/control-measure/values/edit/${item.id}`
        }
        onEdit={() => {}}
        onDelete={() => {}}
      />
