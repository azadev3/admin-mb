...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'registrationsecurity', method: 'get' });
...
const RegistrationsShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/market-participants/banks/register/descriptions/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="registrationsecurity" />
      <DataTable
        ...
        onEditLocation={item =>
          `/market-participants/banks/register/descriptions/edit/${item.id}`
        }
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
