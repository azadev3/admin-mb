...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'registrationsecuritycaption', method: 'get' });
...
const RegistrationsCaptionsShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['registrationsecuritycaption'],
    queryFn: fetchData,
    retry: 2,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/market-participants/banks/register/captions/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="registrationsecuritycaption" />
      <DataTable
...
export default RegistrationsCaptionsShow;
