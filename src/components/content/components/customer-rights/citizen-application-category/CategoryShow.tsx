...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'citizenapplicationcategory', method: 'get' });
  if (!res) return [];
...
const CategoryShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['citizenapplicationcategory'],
    queryFn: fetchData,
    retry: 2,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/customer-rights/citizencategory/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="citizenapplicationcategory" />
      <DataTable
        ...
        refetch={refetch}
        onEditLocation={item => `/customer-rights/citizencategory/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
      />
    </VStack>
