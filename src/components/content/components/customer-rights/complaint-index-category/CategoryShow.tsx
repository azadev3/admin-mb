...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'complaintindexcategory', method: 'get' });
  if (!res) return [];
...
const CategoryShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['complaintindexcategory'],
    queryFn: fetchData,
    retry: 2,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/customer-rights/complaint-index/category/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="complaintindexcategory" />
      <DataTable
        ...
        refetch={refetch}
        onEditLocation={item =>
          `/customer-rights/complaint-index/category/edit/${item.id}`
        }
        onEdit={() => {}}
        onDelete={() => {}}
      />
