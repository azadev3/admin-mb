...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'EconometricModelFile', method: 'get' });
  if (!res) return [];
...
const EconometricModelFileShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/publications/EconometricModelFile/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="EconometricModelFile" />
      <DataTable
        ...
        onEditLocation={item => `/publications/EconometricModelFile/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default EconometricModelFileShow;
