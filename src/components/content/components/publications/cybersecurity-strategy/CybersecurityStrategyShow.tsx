...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'CybersecurityStrategy', method: 'get' });
  if (!res) return [];
...
const CybersecurityStrategyShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['CybersecurityStrategy'],
    queryFn: fetchData,
    retry: 2,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/publications/CybersecurityStrategy/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="CybersecurityStrategy" />
      <DataTable
        ...
        onEditLocation={item => `/publications/CybersecurityStrategy/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default CybersecurityStrategyShow;
