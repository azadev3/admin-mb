...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'FinancialFlow', method: 'get' });
  if (!res) return [];
...
const FinancialFlowShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/publications/FinancialFlow/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="FinancialFlow" />
      <DataTable
        ...
        onEditLocation={item => `/publications/FinancialFlow/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default FinancialFlowShow;
