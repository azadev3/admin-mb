...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'RoadmapOverview', method: 'get' });
  if (!res) return [];
...
const RoadmapOverviewShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/publications/RoadmapOverview/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="RoadmapOverview" />
      <DataTable
        ...
        onEditLocation={item => `/publications/RoadmapOverview/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default RoadmapOverviewShow;
