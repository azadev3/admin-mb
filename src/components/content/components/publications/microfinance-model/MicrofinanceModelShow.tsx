...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'MicrofinanceModel', method: 'get' });
  if (!res) return [];
...
const MicrofinanceModelShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/publications/MicrofinanceModel/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="MicrofinanceModel" />
      <DataTable
        ...
        onEditLocation={item => `/publications/MicrofinanceModel/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default MicrofinanceModelShow;
