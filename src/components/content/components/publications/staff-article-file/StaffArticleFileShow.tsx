...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'StaffArticleFile', method: 'get' });
  if (!res) return [];
...
const StaffArticleFileShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/publications/StaffArticleFile/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="StaffArticleFile" />
      <DataTable
        ...
        onEditLocation={item => `/publications/StaffArticleFile/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default StaffArticleFileShow;
