...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'StaffArticle', method: 'get' });
  if (!res) return [];
...
const StaffArticleShow: React.FC = () => {
  ...
  const dynamicColumns: Column<DataInterface>[] = [
    { header: 'ID', accessor: 'id' },
    {
      header: 'İl',
      accessor: 'year',
      cell: row => (row.year ? <Text>{row.year}</Text> : <Text>Yoxdur</Text>),
    },
  ];
  ...
  allLangs.forEach(lang => {
    ...
    dynamicColumns.push({
      header: `Slogan (${lang.toUpperCase()})`,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/publications/StaffArticle/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="StaffArticle" />
      <DataTable
        ...
        onEditLocation={item => `/publications/StaffArticle/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default StaffArticleShow;
