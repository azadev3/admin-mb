...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'OpenBankingFile', method: 'get' });
  if (!res) return [];
...
const OpenBankingFileShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/financial-innovation/OpenBankingFile/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="OpenBankingFile" />
      <DataTable
        ...
        onEditLocation={item => `/financial-innovation/OpenBankingFile/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default OpenBankingFileShow;
