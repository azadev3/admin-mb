...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'eventvideo', method: 'get' });
  return res && res[0] ? res[0] : [];
};

const EventVideosShow: React.FC = () => {
  ...
  const dynamicColumns: Column<DataInterface>[] = [
    { header: 'ID', accessor: 'id' },
    {
      ...
      cell: row =>
        row.url ? (
          <Link isExternal href={row.url} color="blue.500">
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/eventvideo/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="eventvideo" />
      <DataTable
        ...
        onEditLocation={item => `/eventvideo/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default EventVideosShow;
