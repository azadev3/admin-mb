...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'eventcontent', method: 'get' });
  return res;
};

const EventsContentShow: React.FC = () => {
  ...
  const dynamicColumns: Column<DataInterface>[] = [
    ...
    {
      header: 'Tədbir Başlığı',
      accessor: 'eventTitle',
      cell: row =>
        row.eventTitle ? (
          <Highlighter text={row.eventTitle} highlight={searchTerm} />
        ) : (
          <Text>Yoxdur</Text>
        ),
    },
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/eventcontent/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="eventcontent" />
      <DataTable
        ...
        onEditLocation={item => `/eventcontent/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default EventsContentShow;
