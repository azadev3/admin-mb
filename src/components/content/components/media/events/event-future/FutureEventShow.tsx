...
interface DataInterface {
  ...
  locations: LanguagePayloadShowData;
  formats: LanguagePayloadShowData;
}

const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'futureevent', method: 'get' });
  return res;
};

const FutureEventShow: React.FC = () => {
  ...
  data.forEach(item => {
    ...
    Object.keys(item.formats).forEach(lang => allLangs.add(lang));
    Object.keys(item.locations).forEach(lang => allLangs.add(lang));
    if (item.titles) Object.keys(item.titles).forEach(lang => allLangs.add(lang));
    if (item.locations)
      Object.keys(item.locations).forEach(lang => allLangs.add(lang));
    if (item.formats) Object.keys(item.formats).forEach(lang => allLangs.add(lang));
  });
  ...
  allLangs.forEach(lang => {
    ...
    dynamicColumns.push({
      ...
      accessor: `locations.${lang}`,
      cell: row =>
        row.locations[lang] ? (
          <Highlighter text={row.locations[lang]} highlight={searchTerm} />
        ) : (
          <Text>Yoxdur</Text>
        ),
    });
    dynamicColumns.push({
      ...
      accessor: `formats.${lang}`,
      cell: row =>
        row.formats[lang] ? (
          <Highlighter text={row.formats[lang]} highlight={searchTerm} />
        ) : (
          <Text>Yoxdur</Text>
        ),
    });
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/futureevent/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="futureevent" />
      <DataTable
        ...
        onEditLocation={item => `/futureevent/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default FutureEventShow;
