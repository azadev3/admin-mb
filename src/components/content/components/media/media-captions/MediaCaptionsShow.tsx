...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'mediacaption', method: 'get' });
...
const MediaCaptionsShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/mediacaption/create"
        onRefresh={refetch}
      ...
      <DataTable
        ...
        onEditLocation={item => `/mediacaption/edit/${item.id}`}
      />
    </VStack>
  );
};

export default MediaCaptionsShow;
