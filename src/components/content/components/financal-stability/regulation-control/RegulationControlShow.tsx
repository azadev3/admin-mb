...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'RegulationControl', method: 'get' });
  if (!res) return [];
...
const RegulationControlShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/financial-stability/RegulationControl/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="RegulationControl" />
      <DataTable
        ...
        onEditLocation={item =>
          `/financial-stability/RegulationControl/edit/${item.id}`
        }
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
...
export default RegulationControlShow;
