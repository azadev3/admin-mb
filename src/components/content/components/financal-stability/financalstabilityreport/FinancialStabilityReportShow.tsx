...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'FinancialStabilityReport', method: 'get' });
  if (!res) return [];
...
const FinancialStabilityReportShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['FinancialStabilityReport'],
    queryFn: fetchData,
    retry: 2,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/financial-stability/FinancialStabilityReport/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="FinancialStabilityReport" />
      <DataTable
        ...
        onEditLocation={item =>
          `/financial-stability/FinancialStabilityReport/edit/${item.id}`
        }
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
...
export default FinancialStabilityReportShow;
