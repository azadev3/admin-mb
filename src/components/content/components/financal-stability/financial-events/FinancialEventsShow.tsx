...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'FinancialEvent', method: 'get' });
  if (!res) return [];
...
const FinancialEventsShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/financial-stability/FinancialEvent/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="FinancialEvent" />
      <DataTable
        ...
        onEditLocation={item => `/financial-stability/FinancialEvent/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default FinancialEventsShow;
