...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'monetarypolicygraphic', method: 'get' });
  return res;
};

const GraphicShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['monetarypolicygraphic'],
    queryFn: fetchData,
    retry: 2,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/pul-siyaseti/graphic/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="monetarypolicygraphic" />
      <DataTable
        ...
        onEditLocation={item => `/pul-siyaseti/graphic/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
