...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'monetarypolicydirection', method: 'get' });
  return res;
};

const DirectionShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['monetarypolicydirection'],
    queryFn: fetchData,
    retry: 2,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/pul-siyaseti/direction/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="monetarypolicydirection" />
      <DataTable
        ...
        onEditLocation={item => `/pul-siyaseti/direction/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
