...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'MonetaryPolicyInstrument', method: 'get' });
  return res;
};

const InstrumentShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['MonetaryPolicyInstrument'],
    queryFn: fetchData,
    retry: 2,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/pul-siyaseti/MonetaryPolicyInstrument/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="MonetaryPolicyInstrument" />
      <DataTable
        ...
        onEditLocation={item => `/pul-siyaseti/MonetaryPolicyInstrument/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
