...
const MonetaryPolicyCaptionsShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['monetarypolicycaptions'],
    queryFn: fetchData,
    retry: 2,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/pul-siyaseti/captions/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="monetarypolicycaption" />
      <DataTable
        ...
        onEditLocation={item => `/pul-siyaseti/captions/edit/${item.id}`}
      />
    </VStack>
  );
};

export default MonetaryPolicyCaptionsShow;
