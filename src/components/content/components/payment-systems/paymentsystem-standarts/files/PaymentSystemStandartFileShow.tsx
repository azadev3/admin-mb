...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'PaymentSystemStandartFile', method: 'get' });
  if (!res) return [];
...
const PaymentSystemStandartFileShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['PaymentSystemStandartFile'],
    queryFn: fetchData,
    retry: 2,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/payment-systems/standarts/PaymentSystemStandartFile/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="PaymentSystemStandartFile" />
      <DataTable
        columns={columns}
        data={filteredData}
        ...
        onEditLocation={item => `/payment-systems/standarts/PaymentSystemStandartFile/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default PaymentSystemStandartFileShow;
