...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'PaymentSystemStandart', method: 'get' });
  if (!res) return [];
...
const StandartsShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['PaymentSystemStandart'],
    queryFn: fetchData,
    retry: 2,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/payment-systems/standarts/PaymentSystemStandart/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="PaymentSystemStandart" />
      <DataTable
        columns={columns}
        data={filteredData}
        ...
        onEditLocation={item =>
          `/payment-systems/standarts/PaymentSystemStandart/edit/${item.id}`
        }
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
