...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'PaymentSystemControlFile', method: 'get' });
  if (!res) return [];
...
const PaymentSystemControlFileShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['PaymentSystemControlFile'],
    queryFn: fetchData,
    retry: 2,
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(
      item =>
        ...
        Object.values(item.coverTitles).some(
          val => val && String(val).toLocaleLowerCase('az').includes(lower),
        ),
    );
  }, [searchTerm, data]);
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/payment-systems/PaymentSystemControlFile/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="PaymentSystemControlFile" />
      <DataTable
        columns={columns}
        data={filteredData}
        ...
        onEditLocation={item => `/payment-systems/PaymentSystemControlFile/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default PaymentSystemControlFileShow;
