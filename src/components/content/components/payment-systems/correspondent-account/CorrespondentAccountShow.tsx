...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'CorrespondentAccount', method: 'get' });
  if (!res) return [];
...
const CorrespondentAccountShow: React.FC = () => {
  ...
  const columns: Column<DataInterface>[] = [
    { header: 'ID', accessor: 'id' },
    {
      ...
      accessor: 'file',
      cell: row =>
        row.file ? <Image src={row.file} boxSize={100} /> : <Text>Yoxdur</Text>,
    },
  ];

  const allLangs = new Set<string>();
  data.forEach(item => Object.keys(item.titles).forEach(lang => allLangs.add(lang)));
  data.forEach(item => Object.keys(item.coverTitles).forEach(lang => allLangs.add(lang)));
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/payment-systems/CorrespondentAccount/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="CorrespondentAccount" />
      <DataTable
        columns={columns}
        data={filteredData}
        ...
        onEditLocation={item => `/payment-systems/CorrespondentAccount/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default CorrespondentAccountShow;
