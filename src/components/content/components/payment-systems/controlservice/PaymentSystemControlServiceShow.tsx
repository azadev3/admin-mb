...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'PaymentSystemControlService', method: 'get' });
  if (!res) return [];
...
const PaymentSystemControlServiceShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['PaymentSystemControlService'],
    queryFn: fetchData,
    retry: 2,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/payment-systems/PaymentSystemControlService/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="PaymentSystemControlService" />
      <DataTable
        columns={columns}
        data={filteredData}
        ...
        onEditLocation={item => `/payment-systems/PaymentSystemControlService/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default PaymentSystemControlServiceShow;
