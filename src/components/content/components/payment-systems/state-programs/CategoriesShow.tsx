...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'StateProgramCategory', method: 'get' });
  if (!res) return [];
...
const CategoriesShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/payment-systems/state-programs/categories/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="StateProgramCategory" />
      <DataTable
        columns={columns}
        data={filteredData}
        ...
        onEditLocation={item =>
          `/payment-systems/state-programs/categories/edit/${item.id}`
        }
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
