...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'StateProgram', method: 'get' });
  if (!res) return [];
...
const StateProgramShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/payment-systems/state-programs/StateProgram/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="StateProgram" />
      <DataTable
        columns={columns}
        data={filteredData}
        ...
        onEditLocation={item =>
          `/payment-systems/state-programs/StateProgram/edit/${item.id}`
        }
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
...
export default StateProgramShow;
