...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'NominationCategory', method: 'get' });
  if (!res) return [];
...
const NominationCategoryShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/payment-systems/NominationCategory/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="NominationCategory" />
      <DataTable
        columns={columns}
        data={filteredData}
        ...
        onEditLocation={item => `/payment-systems/NominationCategory/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default NominationCategoryShow;
