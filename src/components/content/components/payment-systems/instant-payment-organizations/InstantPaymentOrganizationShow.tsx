...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'InstantPaymentOrganization', method: 'get' });
  if (!res) return [];
...
const InstantPaymentOrganizationShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['InstantPaymentOrganization'],
    queryFn: fetchData,
    retry: 2,
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(
      item =>
        (item.code && item.code.toLocaleLowerCase('az').includes(lower)) ||
  ...
  const columns: Column<DataInterface>[] = [
    { header: 'ID', accessor: 'id' },
    {
      ...
      cell: row =>
        row.code ? (
          <Highlighter highlight={searchTerm} text={row.code} />
        ) : (
          <Text>Yoxdur</Text>
        ),
    },
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/payment-systems/InstantPaymentOrganization/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="InstantPaymentOrganization" />
      <DataTable
        columns={columns}
        data={filteredData}
        ...
        onEditLocation={item =>
          `/payment-systems/InstantPaymentOrganization/edit/${item.id}`
        }
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
...
export default InstantPaymentOrganizationShow;
