...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'LotteryVideo', method: 'get' });
  if (!res) return [];
...
const LotteryVideoShow: React.FC = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(
      item =>
        ...
        (item.url && item.url.toLocaleLowerCase('az').includes(lower)),
    );
  }, [searchTerm, data]);
  ...
  const columns: Column<DataInterface>[] = [
    { header: 'ID', accessor: 'id' },
    {
      ...
      cell: row =>
        row.url ? (
          <Link color="blue.500" isExternal href={row.url}>
            {row.url}
          </Link>
        ) : (
          <Text>Yoxdur</Text>
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/payment-systems/LotteryVideo/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="LotteryVideo" />
      <DataTable
        columns={columns}
        data={filteredData}
        ...
        onEditLocation={item => `/payment-systems/LotteryVideo/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default LotteryVideoShow;
