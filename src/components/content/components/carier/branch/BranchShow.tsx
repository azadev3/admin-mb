...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'branch', method: 'get' });
  if (!res) return [];
...
const BranchShow: React.FC = () => {
  ...
  const dynamicColumns: Column<DataInterface>[] = [
    { header: 'ID', accessor: 'id' },
    {
      ...
      cell: row =>
        row.department ? (
          <Highlighter highlight={searchTerm} text={row.department} />
        ) : (
          <Text>Yoxdur</Text>
        ),
    },
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/carier/branch/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="branch" />
      <DataTable
        ...
        onEditLocation={item => `/carier/branch/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
