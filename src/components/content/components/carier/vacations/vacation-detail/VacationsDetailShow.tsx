...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'vacancydetail', method: 'get' });
  if (!res) return [];
...
const VacationsDetailShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/carier/vacations/vacation/detail/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="vacancydetail" />
      <DataTable
        ...
        onEditLocation={item => `/carier/vacations/vacation/detail/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default VacationsDetailShow;
