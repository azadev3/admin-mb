...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'internshipprogram', method: 'get' });
  if (!res) return [];
...
const ProgramShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/carier/intern/program/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      {/* <DeleteModal endpoint="internshipprogram" /> */}
      <DataTable
        ...
        // onEditLocation={item => `/carier/intern/program/edit/${item.id}`}
        // onEdit={() => {}}
        // onDelete={() => {}}
        refetch={refetch}
      />
