...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'internshipdirection', method: 'get' });
  if (!res) return [];
...
const DirectionsShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['internshipsdirections'],
    queryFn: fetchData,
    retry: 2,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/carier/intern/directions/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="internshipdirection" />
      <DataTable
        ...
        onEditLocation={item => `/carier/intern/directions/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
