...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'position', method: 'get' });
  if (!res) return [];
...
const PositionShow: React.FC = () => {
  ...
  const dynamicColumns: Column<DataInterface>[] = [
    { header: 'ID', accessor: 'id' },
    {
      ...
      cell: row =>
        row.branch ? (
          <Highlighter highlight={searchTerm} text={row.branch} />
        ) : (
          <Text>Yoxdur</Text>
        ),
    },
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/carier/position/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="position" />
      <DataTable
        ...
        onEditLocation={item => `/carier/position/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
