...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'careercaption', method: 'get' });
...
const CarierCaptionsShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/carier/captions/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="careercaption" />
      <DataTable
        ...
        // onEditLocation={item => `/carier/captions/edit/${item.id}`}
        // onEdit={() => {}}
        // onDelete={() => {}}
        refetch={refetch}
      />
...
export default CarierCaptionsShow;
