...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'nakhchivanbulletin', method: 'get' });
  if (!res) return [];
...
const StatisticalBulletinShow: React.FC = () => {
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/nakhcivan/statistical-bulletin/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="nakhchivanbulletin" />
      <DataTable
        ...
        onEditLocation={item => `/nakhcivan/statistical-bulletin/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default StatisticalBulletinShow;
