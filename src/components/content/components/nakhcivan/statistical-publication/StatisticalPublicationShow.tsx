...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'nakhchivanpublication', method: 'get' });
  if (!res) return [];
...
const StatisticalPublicationShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['statisticalpublication'],
    queryFn: fetchData,
    retry: 2,
  ...
  const dynamicColumns: Column<DataInterface>[] = [
    { header: 'ID', accessor: 'id' },
    {
      ...
      cell: row =>
        row.file ? (
          <Link color="blue.500" isExternal href={row.file}>
            {row.file}
          </Link>
        ) : (
          <Text>Yoxdur</Text>
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/nakhcivan/statistical-publication/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="nakhchivanpublication" />
      <DataTable
        ...
        onEditLocation={item => `/nakhcivan/statistical-publication/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
...
export default StatisticalPublicationShow;
