...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'nakhchivancontact', method: 'get' });
  if (!res) return [];
...
const ContactShow: React.FC = () => {
  ...
  const dynamicColumns: Column<DataInterface>[] = [
    {
      header: 'Map',
      accessor: 'map',
      cell: row => (row.map && row.map[0] ? <Text>{row.map}</Text> : <Text>Yoxdur</Text>),
    },
  ];
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/nakhcivan/contact/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      {/* <DeleteModal endpoint="nakhchivancontact" /> */}
      <DataTable
        ...
        // onEditLocation={item => `/nakhcivan/contact/edit/${item.id}`}
        // onEdit={() => {}}
        // onDelete={() => {}}
        refetch={refetch}
      />
