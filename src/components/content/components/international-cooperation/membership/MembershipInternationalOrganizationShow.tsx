...
const MembershipInternationalOrganizationShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['MembershipInternationalOrganization'],
    queryFn: fetchData,
    retry: 2,
  ...
  const dynamicColumns: Column<DataInterface>[] = [
    { header: 'ID', accessor: 'id' },
    {
      ...
      accessor: 'month',
      cell: row =>
        row.month ? <Text>{Months.find(m => m.value === row.month)?.label ?? ""}</Text> : <Text>Yoxdur</Text>,
    },
    {
      header: 'İl',
      accessor: 'year',
      cell: row => (row.year ? <Text>{row.year?.toString()}</Text> : <Text>Yoxdur</Text>),
    },
  ];
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/international-cooperation/MembershipInternationalOrganization/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="MembershipInternationalOrganization" />
      <DataTable
        ...
        onEditLocation={item =>
          `/international-cooperation/MembershipInternationalOrganization/edit/${item.id}`
        }
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
...
export default MembershipInternationalOrganizationShow;
