...
const FinancialLiteracyCaptionShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['FinancialLiteracyCaption'],
    queryFn: fetchData,
    retry: 2,
  ...
  const dynamicColumns: Column<DataInterface>[] = [
    {
      ...
      accessor: 'url',
      cell: row =>
        row.url ? <Text color="blue.500">{row.url}</Text> : <Text>Yoxdur</Text>,
    },
  ];
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/financial-literacy/FinancialLiteracyCaption/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="FinancialLiteracyCaption" />
      <DataTable
        ...
        // onEditLocation={item => `/financial-literacy/FinancialLiteracyCaption/edit/${item.id}`}
        // onEdit={() => {}}
        // onDelete={() => {}}
        refetch={refetch}
      />
...
export default FinancialLiteracyCaptionShow;
