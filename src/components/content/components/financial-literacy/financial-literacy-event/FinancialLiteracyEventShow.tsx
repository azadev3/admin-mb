...
const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'FinancialLiteracyEvent', method: 'get' });
  if (!res) return [];
...
const FinancialLiteracyEventShow: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['FinancialLiteracyEvent'],
    queryFn: fetchData,
    retry: 2,
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/financial-literacy/FinancialLiteracyEvent/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="FinancialLiteracyEvent" />
      <DataTable
        ...
        onEditLocation={item =>
          `/financial-literacy/FinancialLiteracyEvent/edit/${item.id}`
        }
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
...
export default FinancialLiteracyEventShow;
