...
interface DataInterface {
  ...
  securityTypeTitle: string;
}

const fetchData = async (): Promise<DataInterface[]> => {
  const res = await apiRequest({ endpoint: 'meas', method: 'get' });
...
const MeasShow: React.FC = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    const containsSearch = (val: any): boolean => {
      ...
      if (Array.isArray(val)) {
        return val.some(v => containsSearch(v));
      }

      if (typeof val === 'object') {
        return Object.values(val).some(v => containsSearch(v));
      }
  ...
  const dynamicColumns: Column<DataInterface>[] = [
    ...
    {
      header: 'Qeydiyyat Vaxtı',
      accessor: 'registerTime',
      cell: row => (row.registerTime ? <Text>{moment(row.registerTime).format("DD.MM.YYYY")}</Text> : null),
    },
    {
    ...
    {
      header: 'Emitent Növü',
      accessor: 'issuerTypeTitle',
    },
    {
      header: 'Məlumat Növü',
      accessor: 'informationTypeTitle',
    },
    {
      header: 'Qızıl Kağız Növü',
      accessor: 'securityTypeTitle',
    },
    {
      header: 'FAYL',
      accessor: 'pdfFile',
      cell: row =>
        row.pdfFile ? (
          <Link isExternal href={row.pdfFile} color="blue.500">
            {row.pdfFile}
          </Link>
        ) : (
          <Text>Yoxdur</Text>
  ...
  return (
    <VStack w="100%" align="stretch" spacing={4} p={4} bg="gray.50" borderRadius="md">
      <UserManagement
        createButtonLocation="/meas/MEAS/create"
        onRefresh={refetch}
        dataLoading={isLoading || isFetching}
      />
      <DeleteModal endpoint="meas" />
      <DataTable
        ...
        onEditLocation={item => `/meas/MEAS/edit/${item.id}`}
        onEdit={() => {}}
        onDelete={() => {}}
        refetch={refetch}
      />
